...
export default function PredictionCenter() {
    ...
    return (
        <div className="flex flex-col xl:flex-row min-h-full">
            ...
            <div className="flex-1 p-container-padding space-y-stack-lg pb-stack-lg max-w-6xl">
                <PredictionHeader />
            ...
            {hasResult && predictedEmp && (
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/85 backdrop-blur-sm overflow-y-auto">
                    <div className="bg-secondary-bg border border-border-primary rounded-2xl p-6 space-y-6 max-w-5xl w-full max-h-[90vh] overflow-y-auto relative animate-in fade-in zoom-in-95 duration-200 shadow-2xl">
                        
                        {/* Modal Header */}
                        <div className="flex justify-between items-center border-b border-white/5 pb-4">
                            <div>
                                <h3 className="text-lg font-bold text-white uppercase font-mono">Prediction Analysis Result</h3>
                                <p className="text-xs text-slate-400 font-mono mt-0.5">Target: {predictedEmp.name} ({predictedEmp.id})</p>
                            </div>
                            <button 
                                onClick={() => setHasResult(false)}
                                className="px-4 py-2 rounded bg-primary-bg hover:bg-slate-900 border border-border-primary text-xs uppercase tracking-wider font-mono font-semibold cursor-pointer text-slate-300 transition-colors"
                            >
                                Close Result
                            </button>
                        </div>
